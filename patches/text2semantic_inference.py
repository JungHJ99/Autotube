    ...
    *,
    ...
    for sample_idx in range(num_samples):
        ...
        for batch_idx, batch_text in enumerate(batches):
            ...
            # autotube patch (2026-05-18): upstream reserves 2048 tokens of
            # generation room. On an 8 GB GPU we shrunk max_seq_len to 2560 to
            # keep KV cache within VRAM, so 2048 would leave only 512 tokens of
            # prompt room — too tight for ref-audio (~600 semantic tokens) plus
            # text. We drop the reserved gen room to 1024 (≈ 13 s of audio at
            # ~75 codes/s), which is plenty for chunk_length-clipped requests
            # and gives ~1536 tokens of prompt room.
            _GEN_RESERVE = 1024
            if encoded.size(1) > max_length - _GEN_RESERVE:
                raise ValueError(
                    f"Prompt is too long: {encoded.size(1)} > {max_length - _GEN_RESERVE}"
                )
            ...
            # autotube patch (2026-05-18): free LLAMA generation tensors BEFORE
            # yielding to the consumer (which then runs the audio decoder).
            # Upstream's `del y, encoded` only fires when the generator resumes
            # after the consumer is done, so the decoder OOMs against still-live
            # LLAMA intermediates on an 8 GB GPU.
            del y, encoded
            import gc as _gc
            _gc.collect()
            if torch.cuda.is_available():
